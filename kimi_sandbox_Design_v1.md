...
- 单元测试 151 项全绿（新增 `test_seccomp.py`、`test_paths_v2.py`、`test_bwrap_v2.py`、
  `test_cli_v2.py`；含第二轮 R1/R2/C1、第三轮 R3/R4、第四轮 inode 挂载固定的回归用例）。
  其中 `test_seccomp.py` 内置微型 cBPF 解释器，对多 ABI 拦截行为做**行为级**证明
  （而非仅检查指令形状）。
- smoke 23 项全绿（真实 bubblewrap 0.11.1：原 §28 验收 + v2 功能 + R1 反向覆盖
  + v2.10/v2.11 挂载固定；网络可达性探测对外部依赖做有限重试）。
- CI（`.github/workflows/ci.yml`）在 Python 3.11–3.13 跑单测矩阵，并安装 bubblewrap
  做一次 `--dry-run` 健全性检查（不真正启动 kimi）；完整 smoke（真实 bwrap + 假 kimi 桩
  + 尽力而为的 user-systemd 资源限制）经 `workflow_dispatch` 手动触发。
...
### 39.13 第四轮优化落地（挂载固定 + CI/工程化收尾）

第三轮评审后列出 4 项「可选优化」，本轮全部落地：

- **#3 TOCTOU inode 固定挂载（默认开启）** —— 此前 TOCTOU 仅在 README 记录为残留项。
  现利用 bubblewrap 的 `--bind-fd`/`--ro-bind-fd`：启动器在路径解析+校验**之后**对每个
  宿主 bind 源（project、kimi-code-home、cache、kimi 二进制、额外挂载源）以 `O_PATH`
  打开一个 fd，置 `set_inheritable(True)`，经 `pass_fds` 传给 bwrap，由 bwrap 挂载该 fd
  指向的确切 inode。这样路径在「校验→挂载」之间被换成 symlink 也无效——fd 锁定的是被校验
  的 inode。残留窗口收敛为启动器单次 `open()` 路径行走本身。实现要点：
  - `bwrap.py` 新增 `_bind_args(... path_fds)`，源命中 fd 则发 `--bind-fd`/`--ro-bind-fd`，
    否则回退路径式 `--bind`/`--ro-bind`；`build_bwrap_command` 增 `path_fds` 入参。
  - `cli.py` 新增 `open_bind_fds()`（去重、失败时回收已开 fd 并报 `SandboxError`）；
    run 路径开 fd、组装 `pass_fds`（seccomp + 各 bind fd）、`finally` 全部 `os.close`。
  - 默认开启；`--no-pin-mounts` 关闭（兼容 bwrap < 0.5）。`--dry-run` 始终以**路径形式**
    打印（可读），并在 stderr 注明实跑会用 `--bind-fd` 固定；`--debug` 计划与 banner 增
    `pin mounts: on/off` 行（保持 R2 计划=实跑一致）。
  - 测试：`test_bwrap_v2.py` 4 例（rw/ro fd 形式、部分源固定、无 fd 回退）；
    `test_cli_v2.py` 3 例（默认固定且 fd 全部传入并在返回后关闭、`--no-pin-mounts` 回退、
    dry-run 仍为路径式且有提示）；smoke v2.10/v2.11 在真实 bwrap 下验证计划标记与写盘可见。
    并经真实 bwrap 0.11 手测：fd 固定挂载读写正常。
- **#1 + #4 smoke 进 CI** —— `ci.yml` 新增 `smoke` job，经 `workflow_dispatch` 手动触发：
  安装 bubblewrap、在 PATH 放假 `kimi` 桩（smoke 仅 `--exec`/`--dry-run`，从不真正执行
  kimi），跑完整 `run_smoke.sh`。其中 v2.7 资源限制检查在缺 user-systemd 时自动 SKIP，
  故「资源限制 + seccomp 组合」仅在支持的 runner 上真实执行——即 #1 的尽力而为覆盖。
- **#2 pyproject URLs** —— 由于尚无独立公开仓库，URLs 指向上游 Kimi Code 组织并以 TOML
  注释标注「正式仓库发布后替换」，避免编造不存在的具体仓库路径。

本轮同样不触碰 §37 底线。挂载固定属纯加固：默认行为对用户透明（除依赖 bwrap ≥ 0.5，
已在 README「Requirements」标注并提供 `--no-pin-mounts` 回退）。
